...
(async () => {
  ...
  const outKeys = new Map(out.map(g => [keyOf(g), g]));
  ...
  newOnes.forEach(g => {
    ...
    if (!fullHash(g).startsWith(m[1])) bad.push(g.id + ' (does not match hash of its own fields)');
  });
  check('5. genuinely new gyms each have one unique, non-colliding g- id', newOnes.length === 249 && bad.length === 0, `new records: ${newOnes.length} (expected 249); problems: ${bad.length}${bad.length ? ' e.g. ' + bad[0] : ''}`);
  ...
  for (let run = 0; run < 5; run++) {
    ...
    shuffled.forEach(g => { const h = fullHash(g); for (let len = 10; len <= 40; len += 2) { const id = 'g-' + h.slice(0, len); if (!used.has(id)) { used.add(id); rederived.set(keyOf(g), id); break; } } });
  ...
  check('7. no records silently lost or altered', lost.length === 0 && changed.length === 0 && out.length === expected.length, `expected ${expected.length} (=${orig.length} - ${dropIds.size} documented duplicates); got ${out.length}; lost ${lost.length}; altered ${changed.length}`);
