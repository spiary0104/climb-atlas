-- Baseline migration: the live Climb Atlas database as of 2026-09-24.
--
-- Source: `supabase db dump --linked --schema public` against project thayxaampaelvntoaido (Postgres 17.6),
-- captured read-only in Stage 2 (supabase/introspection/live-2026-09-24/pg_dump-public.sql). It is NOT derived
-- from supabase/schema.sql, which is stale (see docs/schema-introspection-2026-09-24.md).
--
-- This file is a snapshot, not an improvement: it deliberately reproduces live behaviour exactly, including
-- the rate-limit policy, the SECURITY DEFINER function recent_submission_count(), both spots triggers and the
-- table/function grants. Supabase advisor findings are NOT fixed here; they belong in later migrations.
--
-- Removed from the raw dump because the platform / migration role already provides them: session SET lines,
-- CREATE/ALTER/COMMENT SCHEMA public, GRANT USAGE ON SCHEMA public, ALTER DEFAULT PRIVILEGES, and OWNER TO postgres.
-- Extensions (pgcrypto, uuid-ossp, ...) are platform-provided; gen_random_uuid() is built in on PG13+.
--
-- Production already contains all of this. Do NOT run it there: mark it applied instead (see docs/migrations.md).
-- Never edit this file after it is applied anywhere. Add a new migration for every schema change.

-- The SQL-language function recent_submission_count() references public.spots, which is created later in this file
-- (as in the pg_dump it came from), so function bodies must not be validated at creation time.
