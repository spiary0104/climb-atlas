...
and tested. The first batch (`2026-09-24-reconciled-new-gyms`, 246 gyms) was imported to production on 2026-09-24 and verified
(approved spots 1,881 → 2,127; see that batch's `manifest.json`). The match index was rebuilt from production afterwards
(`build-index --live`: 2,127 gyms; the 1,881 earlier entries are byte-identical and the 246 imported ids are added). After any
import, rebuild and commit the index before planning another batch, or its preflight is refused as drift.**
