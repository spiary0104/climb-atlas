...
- What: user said "shanghai next" — fourth of 12 Chinese cities from the
  ...
  split it into two halves rather than do it all in one sitting.
- **Found and worked around a real data-integrity problem**: Beijing gyms
  (confirmed real via web search — e.g. 岩时攀岩's Beijing branch at
  Dawanglu) were mixed into Shanghai's own tab list with no separating
  banner, and at least one card that looked "safe" by scroll position
  (appearing before the obvious contaminated block) turned out to also be
  Beijing on individual verification. Every candidate in this batch was
  therefore verified by city individually, not by scroll position — full
  detail in `docs/architecture.md` "Seed data sourcing", including why
  the "it resolves itself by the end of the scroll" pattern seen in an
  earlier city's video can't be trusted as a general rule.
- **huodong.com has its own genuine 5-page Shanghai climbing directory**
  (`/venue/shanghai/rock_climbing`) — used as the primary source instead
  of photo/position guessing. Pages 1-2 (29 gyms after exclusions) are
  done; pages 3-5 are backlogged (see above).
- This cross-reference caught 4 exact duplicates of gyms already in the
  dataset from an earlier Dianping pass (not re-added) and resolved a
  long-standing "could not confirm a single address" flag on **Banana
  Climbing (Kerry Centre)**. One pair merged as a same-address duplicate
  (RIBBONCLIMBING攀岩馆 / 上海叶岩攀岩馆); one candidate excluded (MELAND
  CLUB, a parent-child entertainment venue, not a dedicated climbing gym).
...
- **Verified**: same offline-fallback-forcing method as every prior batch
  — the Shanghai chip filter returns exactly 37 spots (8 existing + 29
...
- **Not yet done**: running the regenerated SQL against the live
  ...
  Wuhan, Suzhou, Tianjin) — see Backlog. (Shenzhen followed through on
  immediately after, see the entry below.)

### Add Shenzhen (China) — 39 gyms
- Branch: `feature/add-shenzhen-panda` — merged to `master`, pushed.
- Status: done — merged.
- What: user said "keep going" after Guangzhou — seventh of 12 Chinese
  cities from the same "岩馆探索" (PANDA) app footage. Same method again:
  huodong.com's own Shenzhen directory (3 pages, 54 raw candidates)
  instead of frame-extracting `IMG_6365.MP4`.
- **15 of 54 excluded**: 5 exact-address duplicates of already-listed (or
  already-flagged) Banana Climbing entries — incidentally confirming
  those existing entries' addresses. 10 more excluded after reading each
  one's own description: a cave-exploration theme park, a "soft-body
  climbing" kids venue explicitly contrasted from a real rock wall, 2 of
  4 "Little Player Dream Factory" branches whose own pages deny having a
  real climbing wall (the other 2 branches were kept, checked
  individually), a fitness gym with no confirmed wall, a
  basketball-primary hybrid space, and one genuinely kids-only gym
  (Climbing Orangutan, Lvjing) whose own listing explicitly bars adults
  from the climbing zones — the only access-based exclusion, since every
  other youth/family-branded venue kept in this batch lacked that same
  explicit bar.
- **Chain proliferation continued**: Blue Sky Climbing has 7 confirmed
  branches, Climbing Orangutan has 4 (one excluded), Zhengyan/Yijiu/
  Little Warrior each have 2-3, Yanwu Kongjian (already seen in Shanghai)
  got 2 more. Two near-identically-named Cantonese-branded gyms (both
  roughly "there's a place here") were individually confirmed as
  genuinely distinct gyms, not a duplicate listing.
- **Two access/consistency judgment calls kept**: a youth-center Yijiu
  branch only restricts unaccompanied under-8s (no adult bar); two public
  municipal sports-center gyms kept without access caveats, same
  treatment as University Town Sports Center (Guangzhou).
- **Geocoding hit the same severe first-pass failure as Beijing**: only 4
  of 39 resolved on the full-address pass; a simplified second pass
  resolved 31 more; the last 4 needed a metro-station or subdistrict
  fallback, two pairs sharing an identical fallback point (disclosed in
  each spot's own `notes`).
- **Climbing type applied at write time from each gym's own
  description**, same discipline as Beijing/Guangzhou.
...
- Net result: 1379 → **1418 total spots**. Structural check (Node-parsed
  ...
  array.
- **Verified**: same offline-fallback-forcing method as every prior batch
  — the Shenzhen chip filter returns exactly 47 spots (8 existing + 39
  new), no console errors, no mobile horizontal overflow at 375px.
- `supabase_seed_output.sql` regenerated (1418 rows) — still untracked/
  uncommitted by design, a one-off deliverable for the user.
- **Not yet done**: running the regenerated SQL against the live
  Supabase table; the remaining 5 cities (Chengdu, Hangzhou, Wuhan,
  Suzhou, Tianjin) — see Backlog. (Chengdu followed through on
  immediately after, see the entry below.)

### Add Chengdu (China) — 27 gyms
- Branch: `feature/add-chengdu-panda` — merged to `master`, pushed.
- Status: done — merged.
- What: user said "next" after Shenzhen — eighth of 12 Chinese cities
  from the same "岩馆探索" (PANDA) app footage. Same method again:
  huodong.com's own Chengdu directory (3 pages, 37 raw candidates)
  instead of frame-extracting `IMG_6366.MP4`.
- **8 of 37 excluded** after reading each one's own description: a kids
  soft-play "camping" venue explicitly not a dedicated gym, an
  entertainment complex whose "climbing" was climbing nets not a wall,
  a third Meland-branded venue (after Shanghai and Beijing, same
  miscategorized-tag pattern), a go-kart club, a kids-only gym (ages
  3-12, no adult section — same standard as Shenzhen's Climbing
  Orangutan Lvjing exclusion), an outdoor natural-rock base in rural
  Dayi County, a panda-themed kids' play space with no real wall despite
  the category tag, and a university outdoor wall with genuinely
  restrictive (application + approval) access.
- **Two Banana Climbing candidates resolved existing entries rather than
  adding new ones**: one confirmed the already-listed ICD address
  exactly; the other finally resolved a long-standing "no address found"
  flag on Banana Climbing (CapitaLand Tianfu) — updated that entry
  directly, same treatment as Kerry Centre in the Shanghai batch.
- **Chain proliferation continued**: Climbing Panda has 6 confirmed
  branches, Climbing Hero has 2 adult-accessible branches plus 1
  kids-only branch excluded, Flying Frog Climbing has 2. Climbing Panda
  and Super Panda Fitness Studio share panda branding by coincidence,
  confirmed as unrelated chains.
- **One university gym kept with a reservation caveat** (Chengdu Sport
  University Eastern Campus — booking + ID verification, not barred
  outright); **one school-adjacent gym kept without caveats** (Chengdu
  Weiming Climbing Wall — confirmed genuinely public, walk-in hours).
- **Geocoding followed the same two-tier pattern as Beijing/Shenzhen**:
  16 of 27 resolved on the full-address pass, 9 more on a simplified
  pass, the last 2 needed a bare district centroid. Two spots share an
  identical fallback point at the same mall complex (genuinely different
  gyms, not a duplicate).
- **Climbing type applied at write time from each gym's own
...
- Net result: 1418 → **1445 total spots**. Structural check (Node-parsed
  ...
  array.
- **Verified**: same offline-fallback-forcing method as every prior batch
  — the Chengdu chip filter returns exactly 30 spots (3 existing + 27
  new), the resolved CapitaLand Tianfu entry loads with its new address,
  no console errors, no mobile horizontal overflow at 375px.
- `supabase_seed_output.sql` regenerated (1445 rows) — still untracked/
  uncommitted by design, a one-off deliverable for the user.
- **Not yet done**: running the regenerated SQL against the live
...
- Status: done — merged.
- What: user said "keep going" after Xi'an — second of 12 Chinese cities
  from the same "岩馆探索" (PANDA) app footage. Re-extracted frames from
  `IMG_6372.MP4` (originally guessed as Nanjing in the Backlog entry
  above) and found from the frames themselves that it's actually
  Chongqing — see the correction note in the Backlog entry, and full
  sourcing detail in `docs/architecture.md` "Seed data sourcing".
- 35 gyms confirmed via the app's own city banner; 2 matched already-
  listed Chongqing spots exactly (Scream Climbing (Guangdian Park), Black
  Ram Climbing Gym, both from an earlier separate Dianping pass) and were
  left alone; 1 excluded (建设中 tag); 1 excluded (重庆华岩攀岩公园, the same
  outdoor park already excluded during the Dianping pass) — leaving 31
  new gyms.
- **New address source used for this batch**: huodong.com has its own
  Chongqing rock-climbing category page with real per-venue detail pages
  — got a real address for 16 of 31 gyms this way, plus confirmed the 2
  already-listed duplicates. 3 more resolved to a confirmed mall/area
  without an exact unit. The remaining 12 of 31 (39%) had no address
  findable at all and are flagged with a city-centre placeholder — a
  similarly high rate to Xi'an's 47%, confirming this is a real pattern
  for this footage source, not a Xi'an-specific fluke.
- Several genuine multi-branch chains verified (Pandengxia Kids Climbing
  has 7 distinct Chongqing branches, Jidao Climbing has 3, STONE Climbing
  has 3, Lepan Jungle Climbing has 2) — each branch individually
  addressed/flagged rather than assumed to share one location.
- `state` uses the existing `"CHONGQING"` key already in
  `STATES_BY_COUNTRY.CN` — no `js/app.js`, `css/style.css`, or
  `index.html` changes needed, this only appends to `js/data.js`.
- **Same missing-`types` mistake as Xi'an caught and fixed during this
  batch's own verification** (one entry, Chongqing University City Xijie
  Climbing Field) — before it ever reached a live browser this time.
...
- **Verified**: same offline-fallback-forcing method as every prior batch
  ...
  the Chongqing chip filter returns exactly 36 spots (5 existing + 31
  new), no console errors, no mobile horizontal overflow at 375px.
- `supabase_seed_output.sql` regenerated (1231 rows) via the same Node
...
### Add Jeollabuk-do, South Korea (6 gyms, fourth and final division filled)
- Status: done — committed directly to `master`.
- What: direct continuation of the standing "keep working on Korea" task
  — fills the last of South Korea's then-4-empty top-level divisions
  (Gangwon-do, Chungcheongbuk-do, and Gyeongsangbuk-do were completed
  first). Sejong remains excluded, per an earlier session, since its only
  candidate was outdoor-only. **This completes South Korea's full
  division coverage** — every one of the country's 17 real top-level
  divisions now has at least one seed spot. Sourcing (names, addresses,
  discipline evidence) was gathered in earlier segments and all 6
  addresses geocoded via WebFetch against ArcGIS's `findAddressCandidates`
  endpoint (the same substitution used for every prior Korea-division
  batch in this series), all at `score=100` exact matches; this pass
  wrote the `js/data.js`/`css/style.css`/`index.html` entries and
  verified them. Full detail in `docs/architecture.md` "Seed data
  sourcing".
- 6 real, currently-operating gyms across 3 cities: Jeonju Hola Climbing
  Center and Hans Climbing (both Jeonju, Deokjin-gu), Gunsan Climbing
  Center and Gunsan Sports Climbing Center (both Gunsan), ClimbFarm
  (Iksan), and Ascent Climbing Manseong (Jeonju) — all tagged
  bouldering-only per this project's established Korea-specific
  discipline (no independent rope/lead evidence found for any of the
  six).
- **A naming resolution, not a guess**: Jeonju Hola Climbing Center's own
  Facebook page confirms "Hola" and "Olla" are the same gym under two
  romanizations of the same Korean name — recorded rather than treated
...
- **Two genuine open research threads deliberately left unresolved
  ...
  spots' own `notes` fields: a "Hardrock Climbing" cited by secondary
  sources as offering bouldering + lead could not be linked to either
  Jeonju Hola Climbing Center or Hans Climbing despite targeted
  searches; Gunsan Climbing Center's official site only generically
  mentions mountaineering/alpine climbing, with no explicit facility-
  type confirmation, and a second official source could not be fetched.
- Net result: 1875 → **1881 total spots**. Structural check (Node-parsed
  `window.SEED_GYMS`): 1881/1881 unique ids, zero duplicate
  name+suburb+state+country combos, every spot has a non-empty `types`
  array, every KR state code used resolves against `STATES_BY_COUNTRY.KR`.
- **Verified structurally, not via live browser this pass** — this
  session's CDN access was re-confirmed blocked by the environment's
  agent-proxy policy (`connect_rejected` on unpkg.com), matching every
  prior segment's finding; per `Rules.md` §12 this pass went straight to
  the Node-parsed structural check instead of re-litigating the CDN
  block again.
...
- **Not yet done**: running the regenerated seed SQL against the live
  Supabase table — same outstanding step as every prior batch.
