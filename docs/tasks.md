...
- What: direct continuation of the standing "keep working on Korea" task
  ...
  sourcing".
- Net result: 1854 → **1859 total spots**. Structural check and
  offline-fallback-forcing live verification both passed (Gangwon-do
  chip filters to exactly 5 spots, no console errors, no mobile
  overflow) — this pass's live-browser check succeeded cleanly, unlike
  an earlier session's attempt at the same kind of check which hit a CDN
  policy denial and needed a workaround.
